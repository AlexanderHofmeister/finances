...
public final class ClassUtil {
	...
	private static <E> Type getActualTypeArgument(
			final Class<? extends E> instanceClass, final Class<E> superClass,
		...
		for (final Type parentType : parents) {
			...
			if (superClass.equals(parentClass)) {
				...
				return ((ParameterizedType) parentType)
						.getActualTypeArguments()[argumentIndex];
			} else if (parentClass != null
					&& superClass.isAssignableFrom(parentClass)) {
				// Else step up
				final Type type = getActualTypeArgument(
						parentClass.asSubclass(superClass), superClass,
						argumentIndex);
				if (type instanceof Class) {
					return type;
				} else if (type instanceof TypeVariable) {
					return ((ParameterizedType) parentType)
							.getActualTypeArguments()[Arrays
									.asList(parentClass.getTypeParameters())
									.indexOf(type)];
				}
			}
		}
	 ...
	 * @param superClass
	 *            the superclass or interface which specifies the generic type
	 *            variable
	 * @param argumentIndex
	 *            the index of the type variable in the superclass definition (0
	 *            = the first type variable)
	 * @return the bound class for the variable in instanceClass
	 */
	@SuppressWarnings("unchecked")
	public static <T, E> Class<T> getActualTypeBinding(
			final Class<? extends E> instanceClass, final Class<E> superClass,
			final int argumentIndex) {
		Type type = getActualTypeArgument(instanceClass, superClass,
				argumentIndex);
		...
		throw new NoSuchElementException(
				"Can't find binding for the " + argumentIndex + ". argument of "
